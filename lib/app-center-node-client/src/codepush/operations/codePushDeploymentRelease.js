 ...
 */
function _rollback(appName, deploymentName, ownerName, options, callback) {
  ...
  let releaseLabel = (options && options.releaseLabel !== undefined) ? options.releaseLabel : undefined;
  // Validate
  try {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/deployments/{deployment_name}/rollback_release';
...
class CodePushDeploymentRelease {
   ...
   */
  rollbackWithHttpOperationResponse(appName, deploymentName, ownerName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._rollback(appName, deploymentName, ownerName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  rollback(appName, deploymentName, ownerName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._rollback(appName, deploymentName, ownerName, options, optionalCallback);
    }
  }
