 ...
 */
function _profile(appName, ownerName, releaseId, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/releases/{release_id}/provisioning_profile';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['ProvisioningProfileResponse']().mapper();
...
/** Class representing a Provisioning. */
class Provisioning {
  /**
   ...
   */
  profileWithHttpOperationResponse(appName, ownerName, releaseId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._profile(appName, ownerName, releaseId, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  profile(appName, ownerName, releaseId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._profile(appName, ownerName, releaseId, options, optionalCallback);
    }
  }
