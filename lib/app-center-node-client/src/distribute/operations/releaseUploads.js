...
function _create(appName, ownerName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/release_uploads';
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _complete(appName, body, ownerName, uploadId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/release_uploads/{upload_id}';
  requestUrl = requestUrl.replace('{app_name}', encodeURIComponent(appName));
  requestUrl = requestUrl.replace('{owner_name}', encodeURIComponent(ownerName));
...
/** Class representing a ReleaseUploads. */
class ReleaseUploads {
  /**
   * Create a ReleaseUploads.
   ...
   * @reject {Error} - The error object.
   */
  completeWithHttpOperationResponse(appName, body, ownerName, uploadId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._complete(appName, body, ownerName, uploadId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  complete(appName, body, ownerName, uploadId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._complete(appName, body, ownerName, uploadId, options, optionalCallback);
    }
  }
