...
async function setup(): Promise<void> {
    ...
    if (!testParams.RunBasicTests) {
        ...
        if (process.platform === "win32") {
            SetupEnvironmentHelper.prepareRNWApplication(RNWWorkspaceFilePath, resourcesPath, RNWWorkspacePath, SmokeTestsConstants.RNWAppName, "RNWSample", process.env.RNW_VERSION);
        }
