...
export function startExpoTests(expoWorkspace: string, pureWorkspace: string, testParameters: TestRunArguments): void {
    ...
    describe("Expo tests", () => {
        ...
        async function findExpoSuccessAndFailurePatterns(filePath: string, successPattern: string, failurePattern: string): Promise<ExpoLaunch> {
        ...
        async function runExpoDebugScenario(logFilePath: string, testName: string, workspacePath: string, debugConfigName: string, triesToLaunchApp: number) {
            ...
            if (triesToLaunchApp <= 1) {
                ...
                for (let retry = 1; retry <= triesToLaunchApp; retry++) {
                    ...
                    await app.workbench.quickaccess.runDebugScenario(debugConfigName);
                    expoLaunchStatus = await findExpoSuccessAndFailurePatterns(logFilePath, ExpoSuccessPattern, ExpoFailurePattern);
        ...
        async function expoTest(appFileName: string, testName: string, workspacePath: string, debugConfigName: string, platform: Platform.AndroidExpo | Platform.iOSExpo, triesToLaunchApp: number) {
            ...
            if (platform === Platform.iOSExpo) {
                ...
                if (!appFile) {
                    throw new Error(`iOS Expo app is not found in ${SmokeTestsConstants.iOSExpoAppsCacheDir}`);
                }
        ...
        if (testParameters.RunAndroidTests) {
            ...
            it("Android Pure RN app Expo test(LAN)", async function () {
                this.timeout(debugExpoTestTime);
                await expoTest("App.js", "Android pure RN Expo test(LAN)", pureWorkspace, ExpoLanDebugConfigName, Platform.AndroidExpo, 1);
            });

            it("Android Expo app Debug test(LAN)", async function () {
                this.timeout(debugExpoTestTime);
                await expoTest("App.tsx", "Android Expo Debug test(LAN)", expoWorkspace, ExpoLanDebugConfigName, Platform.AndroidExpo, 1);
            });
