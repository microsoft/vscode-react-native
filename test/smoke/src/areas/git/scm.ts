...
export class SCM extends Viewlet {
    ...
    public async openChange(name: string): Promise<void> {
        await this.spectron.client.waitAndClick(SCM_RESOURCE_CLICK(name));
    }

    public async stage(name: string): Promise<void> {
    ...
    public async unstage(name: string): Promise<void> {
