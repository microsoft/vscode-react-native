...
export class Extensions extends Viewlet {
    ...
    async installExtension(id: string, waitUntilEnabled: boolean): Promise<void> {
        ...
        if (waitUntilEnabled) {
            await this.code.waitForElement(
                `.extension-editor .monaco-action-bar .action-item:not(.disabled) .extension-action[title="Disable this extension"]`,
            );
        }
    }
    ...
    async closeExtension(title: string): Promise<any> {
        await this.code.waitAndClick(
            `.tabs-container div.tab[title="Extension: ${title}"] div.tab-actions a.action-label.codicon.codicon-close`,
        );
    }
}
