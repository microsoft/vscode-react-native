...
function buildDriver(browser: playwright.Browser, page: playwright.Page): IDriver {
    const driver: IDriver = {
        ...
        setValue: async (windowId, selector, text) =>
            page.evaluate(`window.driver.setValue('${selector}', '${text}')`).then(undefined),
        getTitle: windowId => page.evaluate(`window.driver.getTitle()`),
        isActiveElement: (windowId, selector) =>
            page.evaluate(`window.driver.isActiveElement('${selector}')`),
        ...
        typeInEditor: (windowId, selector, text) =>
            page.evaluate(`window.driver.typeInEditor('${selector}', '${text}')`),
        getTerminalBuffer: (windowId, selector) =>
            page.evaluate(`window.driver.getTerminalBuffer('${selector}')`),
        writeInTerminal: (windowId, selector, text) =>
            page.evaluate(`window.driver.writeInTerminal('${selector}', '${text}')`),
    };
    return driver;
}
...
export async function launch(
    ...
    codeServerPath = process.env.VSCODE_REMOTE_SERVER_PATH,
): Promise<void> {
    ...
    if (codeServerPath) {
        serverLocation = join(
            codeServerPath,
            `server.${process.platform === "win32" ? "cmd" : "sh"}`,
        );
        ...
        serverLocation = join(
            ...
            "..",
            "..",
            `resources/server/web.${process.platform === "win32" ? "bat" : "sh"}`,
        );
        console.log(`Starting server out of sources from "${serverLocation}"`);
    }
    server = spawn(serverLocation, ["--browser", "none", "--driver", "web"], { env });
...
export function connect(
    browserType: "chromium" | "webkit" | "firefox" = "chromium",
): Promise<{ client: IDisposable; driver: IDriver }> {
    return new Promise(async c => {
        ...
        await page.goto(
            `${endpoint}&folder=vscode-remote://localhost:9888${URI.file(workspacePath!).path}`,
        );
        const result = {
