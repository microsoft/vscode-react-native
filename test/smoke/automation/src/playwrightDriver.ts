        ...
        exitApplication: async () => {
            try {
                await context.tracing.stop({
                    path: join(logsPath, `playwright-trace-${traceCounter++}.zip`),
                });
            } catch (error) {
...
export async function launch(
    ...
    server = spawn(serverLocation, args, { env });
    ...
    return new Promise(async c => {
        ...
        page.on("response", async response => {
            if (response.status() >= 400) {
                console.error(
                    `Playwright ERROR: HTTP status ${response.status()} for ${response.url()}`,
                );
            }
        });
        const payloadParam = `[["enableProposedApi",""],["skipWelcome","true"]]`;
        await page.goto(
            `${endpoint}&folder=vscode-remote://localhost:9888${
