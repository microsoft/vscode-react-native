...
* Extension debugger verbose logs (these logs are shown up if only `trace: "verbose"` option is enabled in debug scenarios)
    * React Native Chrome Proxy - shows what runs in and out to the debugger and application
    * Debug Console become more informative and contain some debugging information from the debug adapter
