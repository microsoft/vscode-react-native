...
|`target`|Target to run on. Possible values: `simulator`, `device`, `device=<iOS device name>`, [`<Android emulator/device id>`](https://github.com/react-native-community/cli/blob/master/docs/commands.md#--deviceid-string), `<Android emulator name>`, `<iOS simulator name>`. If value is `simulator` then quick pick window will be expanded with the names of the available virtual devices, then target value in `launch.json` will be changed to name of selected virtual device. If you have only one virtual device available, it will be selected automatically.|`string`|`simulator`|
