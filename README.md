...
* React Native - main extension channel which collects outputs from React Native Packager and shows critical errors in the extension
* These channels spawns only when the specific launch scenario is executed:
...
* Debugger verbose logs (these logs are shown up if only `trace: "verbose"` option is enabled in debug scenarios)
    * React Native Chrome Proxy - shows what runs in and out to the debugger and application (back and forth)
    * In stable version expanded logs are printed into Debug Console
...
There are also some global extension technical logs that might be exposed to the output. To do that set the following properties:
