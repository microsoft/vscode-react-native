...
The extension supports running through Exponent not just the applications with Expo SDK but even pure React Native applications, but you will need to add expo to your `package.json` using this command `npm i --save-dev expo`. In either cases it uses `app.json` configuration file in the root of the project.
