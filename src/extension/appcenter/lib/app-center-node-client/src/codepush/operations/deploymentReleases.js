 ...
 */
function _update(appName, deploymentName, ownerName, release, releaseLabel, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/deployments/{deployment_name}/releases/{release_label}';
...
/** Class representing a DeploymentReleases. */
class DeploymentReleases {
  /**
   * Create a DeploymentReleases.
   ...
   */
  updateWithHttpOperationResponse(appName, deploymentName, ownerName, release, releaseLabel, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._update(appName, deploymentName, ownerName, release, releaseLabel, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  update(appName, deploymentName, ownerName, release, releaseLabel, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._update(appName, deploymentName, ownerName, release, releaseLabel, options, optionalCallback);
    }
  }
