...
export class AndroidPlatform extends GeneralMobilePlatform {
    ...
    public runApp(shouldLaunchInAllDevices = false): Promise<void> {
        let extProps: any = {
            platform: {
                value: "android",
                isPii: false,
    ...
    private getTargetEmulator(devices: IDevice[]): IDevice {
        const activeFilterFunction = (device: IDevice) => {
            return device.isOnline;
        };

        const targetFilterFunction = (device: IDevice) => {
        ...
        const activeDevices = devices && devices.filter(activeFilterFunction);
