...
export function onChangeConfiguration(event: vscode.ConfigurationChangeEvent): void {
    if (event.affectsConfiguration(PACKAGER_PORT_CONFIGURATION)) {
        updatePackagerPortFromSettings();
    }
}

function updatePackagerPortFromSettings(): void {
    const packagers = Object.values(ProjectsStorage.projectsCache).map(appLauncher =>
        appLauncher.getPackager(),
    );
    packagers.forEach(packager => {
        const status = packager.getPackagerStatus();
        const isRunningPackager =
            status === PackagerStatus.PACKAGER_STARTED ||
            status === PackagerStatus.PACKAGER_STARTING ||
            status === PackagerStatus.PACKAGER_STOPPING;

        if (isRunningPackager) {
            const settingsPort = SettingsHelper.getPackagerPort(packager.getProjectPath());
            const pendingPort = settingsPort === packager.getPort() ? undefined : settingsPort;
            packager.getStatusIndicator().setPendingPackagerPort(pendingPort);
            return;
        }

        packager.resetToSettingsPort();
    });
}
