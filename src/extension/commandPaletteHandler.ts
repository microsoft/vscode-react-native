...
export class CommandPaletteHandler {
                ...
                return this.executeCommandInContext(
                    ...
                    () => {
                        const platform = <AndroidPlatform>(
                            this.createPlatform(appLauncher, "android", AndroidPlatform, target)
                        );
                        return platform
                            .beforeStartPackager()
                            .then(() => {
                ...
                return this.executeCommandInContext(
                    ...
                    () => {
                        const platform = <IOSPlatform>(
                            this.createPlatform(appLauncher, "ios", IOSPlatform, target)
                        );
                        return platform
                            .beforeStartPackager()
                            .then(() => {
                    ...
                    return this.executeCommandInContext(
                        ...
                        () => {
                            ...
                            const platform = <ExponentPlatform>(
                                this.createPlatform(appLauncher, "exponent", ExponentPlatform)
                            );
                            return platform
                                .beforeStartPackager()
                                .then(() => {
    ...
    public static showDevMenu(): Promise<void> {
        return this.selectProject().then((appLauncher: AppLauncher) => {
            const androidPlatform = <AndroidPlatform>(
                this.createPlatform(appLauncher, "android", AndroidPlatform)
            );
            androidPlatform.showDevMenu().catch(() => {}); // Ignore any errors

            if (process.platform === "darwin") {
                const iosPlatform = <IOSPlatform>(
                    this.createPlatform(appLauncher, "ios", IOSPlatform)
                );
                iosPlatform.showDevMenu(appLauncher).catch(() => {}); // Ignore any errors
            }
    ...
    public static reloadApp(): Promise<void> {
        return this.selectProject().then((appLauncher: AppLauncher) => {
            ...
            androidPlatform.reloadApp().catch(() => {}); // Ignore any errors

            if (process.platform === "darwin") {
                const iosPlatform = <IOSPlatform>(
                    this.createPlatform(appLauncher, "ios", IOSPlatform)
                );
                iosPlatform.reloadApp(appLauncher).catch(() => {}); // Ignore any errors
            }
            return Promise.resolve();
        });
    }
