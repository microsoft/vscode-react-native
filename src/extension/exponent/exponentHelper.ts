...
export class ExponentHelper {
    ...
    public isExpoApp(showProgress = false): Promise<boolean> {
        if (showProgress) {
            this.logger.logStream("...");
        }
...
AppRegistry.registerRunnable('main', function(appParameters) {
    ...
    private patchAppJson(isExpo = true): Promise<void> {
        return this.readAppJson()
            ...
            .then((config: AppJson) => {
                const expoConfig = <ExpConfig>(config.expo || {});
     ...
     */
    private exponentSdk(showProgress = false): Promise<string> {
        if (showProgress) {
            this.logger.logStream("...");
        }
