...
export class PromiseUtil<T> {
     ...
     */
    public retryAsync<T>(operation: () => Q.Promise<T>, condition: (result: T) => Q.Promise<boolean>, maxRetries: number, delay: number, failure: string): Q.Promise<T> {
        return this.retryAsyncIteration(operation, condition, maxRetries, 0, delay, failure);
    }

    private retryAsyncIteration<T>(operation: () => Q.Promise<T>, condition: (result: T) => Q.Promise<boolean>, maxRetries: number, iteration: number, delay: number, failure: string): Q.Promise<T> {
        return operation()
            .then(result => {
